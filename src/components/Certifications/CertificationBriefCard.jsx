...
export default function CertificationBriefCard({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div
        className="bg-[#111827] border border-slate-800 rounded-2xl 
                    w-full max-w-lg 
                    max-h-[90vh] flex flex-col"
      >
        {/* Header (Always Visible) */}
        <div className="flex justify-between px-6 py-5 border-b border-slate-800 shrink-0">
            ...
            <div className="bg-[#0B1220] p-4 rounded-xl flex flex-col gap-4">
              ...
              {previewUrl &&
                (item.fileUrl.endsWith(".pdf") ? (
                  <iframe
                    src={previewUrl}
                    className="w-full h-87.5 rounded-lg border border-slate-700"
                  ...
                  <img
                    src={previewUrl}
                    alt="Certificate"
                    className="w-full max-h-87.5 object-contain rounded-lg border border-slate-700"
                  />
                ))}
            </div>
          )}
