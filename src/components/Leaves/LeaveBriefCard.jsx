...
//   );
// }
import { X, Calendar, MessageCircle, Loader2, FileText } from "lucide-react";
import { useContext, useState } from "react";
import { AuthContext } from "../../context/AuthContext";
      ...
      <div className="bg-[#111827] border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200">
        ...
        <div className="flex items-start justify-between px-6 py-5 border-b border-slate-800">
          <div>
            ...
            <p className="text-sm text-slate-400">{leave.email}</p>
          </div>
          <button
            onClick={onClose}
            disabled={loadingAction}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between bg-[#0B1220] border border-slate-800 rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">No. of Days: {days}</p>
              </div>
            </div>
            <span
              className={`px-3 py-1 rounded-lg text-xs font-bold uppercase ${statusColors[leave.status]}`}
          ...
          <div className="space-y-2">
            <p className="text-xs uppercase font-semibold text-slate-400 flex items-center gap-2">
              <FileText size={14} /> Supporting Document
            </p>
            <div className="bg-[#0B1220] border border-slate-800 rounded-xl p-4 text-sm text-indigo-400">
              {leave.supportingDocumentPath ? (
                <a
                  href={leave.supportingDocumentPath}
                  target="_blank"
          ...
          <div className="space-y-1 text-xs text-slate-500">
            <p>Created At: {formatDateTime(leave.createdAt)}</p>
            <p>Updated At: {formatDateTime(leave.updatedAt)}</p>
          </div>
          ...
          {user?.role === "MANAGER" && leave.status === "PENDING" && (
            <div className="flex gap-3 pt-4">
              <button
                onClick={handleReject}
                disabled={loadingAction}
                className="flex-1 px-4 py-3 rounded-xl text-sm font-semibold text-rose-400 bg-rose-500/10 hover:bg-rose-500/20 transition disabled:opacity-60 flex items-center justify-center gap-2"
              >
                {loadingAction === "reject" && (
                  <Loader2 size={16} className="animate-spin" />
                )}
              ...
              <button
                onClick={handleApprove}
                disabled={loadingAction}
                className="flex-1 px-4 py-3 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 transition active:scale-95 disabled:opacity-60 flex items-center justify-center gap-2"
              >
                {loadingAction === "approve" && (
                  <Loader2 size={16} className="animate-spin" />
                )}
