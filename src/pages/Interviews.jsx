...
export default function InterviewsComponent() {
  ...
  const badgeStyles = {
    ...
    RESCHEDULE: "bg-indigo-500/10 text-indigo-400",
  };
              ...
              {interviews.map((item) => (
                <tr key={item.id} className="hover:bg-[#0B1220] transition">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-indigo-600/10 flex items-center justify-center text-indigo-400 font-bold text-xs uppercase">
                        {item.intervieweeEmail.charAt(0)}
                      </div>
      ...
      {user?.role === "HR" && (
        <div className="space-y-6 pt-10">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <User className="text-indigo-400" /> Admin Oversight
          </h2>
          ...
          <div className="bg-[#111827] border border-slate-800 rounded-2xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {employeeInterviews.map((item) => (
                  <tr key={item.id} className="hover:bg-[#0B1220] transition">
                    <td className="px-6 py-6 font-semibold text-slate-200">
                      {item.intervieweeEmail}
                    </td>
                    ...
                    <td className="px-6 py-6 text-center">
                      {item.status === "RESCHEDULE" ? (
                        <button
                          ...
                          className="text-xs font-bold text-indigo-400 uppercase"
                        >
                          Set Slot
                        </button>
                      ) : (
                        <button className="p-2 text-slate-400 hover:text-rose-400">
                          <MoreHorizontal size={18} />
                        </button>
                      )}
                    </td>
